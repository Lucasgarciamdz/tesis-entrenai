...
from src.entrenai.api.models import DocumentChunk # Assuming this model is still relevant
...
class PgvectorWrapper:
    ...
    def __init__(self, config: PgvectorConfig):
        ...
        self.cursor = None
        try:
            if not all([config.host, config.port, config.user, config.password, config.db_name]):
            ...
            logger.error(f"An unexpected error occurred during PgvectorWrapper initialization: {e}")
            self.conn = None
            self.cursor = None
    ...
    def _normalize_name_for_table(self, name: str) -> str:
        ...
        name_processed = re.sub(r'[^a-z0-9_]', '', name_processed) # PostgreSQL allows only alphanumeric and underscore
        name_processed = name_processed[:50] # Max table name length is typically 63, give some room for prefix
        
        if not name_processed:
            logger.error(f"Normalized name for '{name}' resulted in empty string. Using fallback.")
            raise ValueError(f"Course name '{name}' resulted in an empty normalized table name.")
    ...
    def ensure_table(self, course_name: str, vector_size: int) -> bool:
        ...
        try:
            ...
            self.conn.commit()
            logger.info(f"Table '{table_name}' created successfully with vector size {vector_size}.")
            return True
        except psycopg2.Error as e:
        ...
        try:
            for chunk in chunks:
                if chunk.embedding is None:
                    logger.warning(f"Chunk with ID '{chunk.id}' (course: {chunk.course_id}, doc: {chunk.document_id}) has no embedding. Skipping.")
                    continue
                ...
                self.cursor.execute(sql, (
                    chunk.id,
                    str(chunk.course_id), # Ensure course_id is string if it's treated as such
                    chunk.document_id,
                    chunk.text,
            ...
            self.conn.commit()
            if upserted_count > 0:
                logger.info(f"Successfully upserted {upserted_count} chunks into table '{table_name}'.")
            else:
                logger.info(f"No valid chunks with embeddings were found to upsert into '{table_name}'.")
            return True
        except psycopg2.Error as e:
            ...
            logger.error(f"Unexpected error upserting chunks into table '{table_name}': {e}")
            if self.conn: self.conn.rollback()
            return False
    ...
    ) -> List[Dict[str, Any]]: # Return type matches Qdrant's ScoredPoint-like structure
        """
            ...
            for row in results:
                formatted_results.append({
                    ...
                    "payload": { # Reconstruct a payload similar to Qdrant's
                        ...
                        "text": row.get("text"),
                        **(row.get("metadata") if isinstance(row.get("metadata"), dict) else {}) # Spread metadata if it's a dict
                    }
                })

            logger.info(f"Search in table '{table_name}' found {len(formatted_results)} results.")
