...
def test_pgvector_wrapper_initialization(
    mocked_pgvector_wrapper: PgvectorWrapper,
    ...
    assert wrapper.cursor.execute.called, (
        "cursor.execute was not called during initialization (for file_tracker table)"
    )
    ...
    for call_args in wrapper.cursor.execute.call_args_list:
        ...
        if (
            f"SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = '{wrapper.FILE_TRACKER_TABLE_NAME}')"
            in sql
        ):
            found_table_check_call = True
            break
    assert found_table_check_call, (
        f"Did not find expected table existence check for {wrapper.FILE_TRACKER_TABLE_NAME}"
    )

    # Check connection log message
    assert (
        f"PgvectorWrapper initialized and connected to {mock_pgvector_config.host}:{mock_pgvector_config.port}/{mock_pgvector_config.db_name}"
        in caplog.text
    )
    ...
    assert (
        f"Table '{wrapper.FILE_TRACKER_TABLE_NAME}'" in caplog.text
    )  # Could be "already exists" or "created"
...
def test_is_file_new(mocked_pgvector_wrapper: PgvectorWrapper):
    ...
    assert (
        wrapper.is_file_new_or_modified(course_id, file_identifier, moodle_timemodified)
        is True
    )
...
def test_mark_file_as_processed_and_check_not_new(
    mocked_pgvector_wrapper: PgvectorWrapper,
):
    wrapper = mocked_pgvector_wrapper
    course_id = 1
    ...
    wrapper.cursor.fetchone.return_value = {
        "moodle_timemodified": moodle_ts
    }  # Simulate file found with same timestamp
...
def test_is_file_not_modified_older_timestamp(mocked_pgvector_wrapper: PgvectorWrapper):
    ...
    assert (
        wrapper.is_file_new_or_modified(course_id, file_id, current_moodle_ts) is False
    )

    # Reset mock for the next call with older timestamp
    wrapper.cursor.reset_mock()  # Reset call count for execute
    wrapper.cursor.fetchone.return_value = {
        "moodle_timemodified": current_moodle_ts
    }  # Still returns the stored current_ts
...
def test_is_file_new_or_modified_db_error(
    mocked_pgvector_wrapper: PgvectorWrapper, caplog
):
    wrapper = mocked_pgvector_wrapper
    wrapper.cursor.execute.side_effect = psycopg2.Error(
        "Simulated DB error for is_file_new_or_modified"
    )

    assert (
        wrapper.is_file_new_or_modified(1, "error_file.pdf", int(time.time())) is True
    )  # Conservative return
...
def test_mark_file_as_processed_db_error(
    ...
    wrapper = mocked_pgvector_wrapper
    wrapper.cursor.execute.side_effect = psycopg2.Error(
        "Simulated DB error for mark_file_as_processed"
    )
...
def test_get_processed_files_timestamps_db_error(
    mocked_pgvector_wrapper: PgvectorWrapper, caplog
):
    wrapper = mocked_pgvector_wrapper
    wrapper.cursor.execute.side_effect = psycopg2.Error(
        "Simulated DB error for get_processed_files_timestamps"
    )
...
def test_ensure_file_tracker_table_db_error_during_init_check(
    mock_pgvector_config, caplog
):
    """
    ...
    mock_cursor.execute.side_effect = psycopg2.Error(
        "Simulated DB error during table check"
    )
    ...
    assert (
        f"Error ensuring table '{PgvectorWrapper.FILE_TRACKER_TABLE_NAME}'"
        in caplog.text
    )
    mock_conn.rollback.assert_called_once()


def test_ensure_file_tracker_table_already_exists(
    mocked_pgvector_wrapper: PgvectorWrapper, caplog
):
    """
    ...
    for call_args in wrapper.cursor.execute.call_args_list:
        ...
        if "CREATE TABLE" in sql:
            ...
            break
    assert not create_call_found, (
        "Should not have attempted to CREATE TABLE if it already exists."
    )
...
def test_no_db_connection_scenario_for_methods(mock_pgvector_config, caplog):
    ...
    with patch(
        "psycopg2.connect",
        side_effect=psycopg2.OperationalError("Simulated connection failure"),
    ):
        with patch(
            "src.entrenai.core.db.pgvector_wrapper.register_vector", MagicMock()
        ):
    ...
    assert (
        wrapper.is_file_new_or_modified(1, "file.txt", 123) is True
    )  # Conservative return
    ...
    assert (
        "No database connection. Cannot get processed files timestamps." in caplog.text
    )
    caplog.clear()
