...
from unittest.mock import patch, MagicMock, ANY, call # ANY and call for more flexible assertions
# Removed Qdrant specific imports like Distance, VectorParams, PointStruct, UpdateStatus

from src.entrenai.core.db.pgvector_wrapper import PgvectorWrapper, PgvectorWrapperError # Updated imports
...
def pgvector_wrapper_with_mock_connection(
...
) -> tuple[PgvectorWrapper, MagicMock, MagicMock]: # Return wrapper, mock_conn, mock_cursor
    mock_conn = MagicMock()
    ...
    mock_cursor.fetchone.return_value = {'exists': True} # For potential table checks during init if any
...
def test_pgvector_wrapper_initialization_connection_error(
    mock_register_vector, mock_psycopg2_connect, mock_pgvector_config: PgvectorConfig, caplog
):
    ...
    assert f"Failed to connect to PostgreSQL/pgvector at {mock_pgvector_config.host}:{mock_pgvector_config.port}: Connection failed" in caplog.text
...
def test_get_table_name( # Renamed test
    ...
    expected_table_name = f"{wrapper.config.collection_prefix}{expected_normalized_name}"
    ...
    expected_table_name_chars = f"{wrapper.config.collection_prefix}{expected_normalized_chars}"
    assert wrapper.get_table_name(course_name_with_special_chars) == expected_table_name_chars
...
def test_ensure_table_creates_if_not_exists(
    ...
    mock_cursor.execute.assert_any_call("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = %s);", (table_name,))
    ...
    mock_cursor.execute.assert_any_call(ANY) # First call is existence check, then CREATE TABLE
    ...
    assert any(normalize_sql(expected_create_table_sql) == normalize_sql(sql) for sql in executed_sqls)

    # Check for CREATE INDEX statement
    expected_create_index_sql = f"CREATE INDEX ON {table_name} USING hnsw (embedding vector_cosine_ops);"
    assert any(normalize_sql(expected_create_index_sql) == normalize_sql(sql) for sql in executed_sqls)
...
def test_ensure_table_exists( # Renamed and simplified
    ...
    mock_cursor.execute.assert_called_once_with("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = %s);", (table_name,))
    
    # Ensure no CREATE TABLE or CREATE INDEX calls were made
    executed_sqls = [call_args[0][0].upper() for call_args in mock_cursor.execute.call_args_list]
    assert not any("CREATE TABLE" in sql for sql in executed_sqls if sql != "SELECT EXISTS (SELECT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = %S);")
...
def test_ensure_table_handles_db_error( # Renamed
    pgvector_wrapper_with_mock_connection: tuple[PgvectorWrapper, MagicMock, MagicMock], caplog
):
    wrapper, mock_conn, mock_cursor = pgvector_wrapper_with_mock_connection
...
def test_upsert_chunks_success(
    ...
    chunks_to_upsert = [
        ...
        DocumentChunk(
            ...
            text="text2",
            embedding=[0.2] * 384,
            metadata={"key": "val2", "another_key": {"nested": "value"}}, # Test with nested metadata
        ),
    ]
    
    assert wrapper.upsert_chunks(course_name, chunks_to_upsert) is True
    
    wrapper.ensure_table.assert_called_once_with(course_name, 384) # Assuming default_vector_size or derived
    ...
    assert mock_cursor.execute.call_count >= len(chunks_to_upsert) # At least one execute per chunk
...
def test_upsert_chunks_empty_list(
    ...
    insert_calls = [c for c in mock_cursor.execute.call_args_list if "INSERT INTO" in c.args[0]]
...
def test_upsert_chunks_with_none_embeddings(
    ...
    chunks_with_none_emb = [
        DocumentChunk(
            id="id1", course_id=course_name, document_id="doc1", text="text1",
            embedding=[0.1] * 384, metadata={},
        ),
        DocumentChunk( # This one will be skipped
            id="id2", course_id=course_name, document_id="doc1", text="text2",
            embedding=None, metadata={},
        ),
        DocumentChunk(
            id="id3", course_id=course_name, document_id="doc1", text="text3",
            embedding=[0.3] * 384, metadata={},
        ),
    ]
    ...
    assert "Chunk with ID 'id2' (course: NoneEmbCourse, doc: doc1) has no embedding. Skipping." in caplog.text
    mock_conn.commit.assert_called_once()
...
def test_upsert_chunks_db_error( # Renamed
    ...
    chunks = [
        DocumentChunk(id="id1", course_id=course_name, document_id="d1", text="t1", embedding=[0.1]*384, metadata={})
    ]
    ...
    assert f"Error upserting chunks into table '{table_name}': DB Upsert Error" in caplog.text
    mock_conn.rollback.assert_called_once()
...
def test_search_chunks_success(
    ...
    mock_db_results = [
        {"id": "hit1_id", "score": 0.9, "course_id": course_name, "document_id": "doc1", "text": "found text 1", "metadata": {"source": "file1.pdf"}},
        {"id": "hit2_id", "score": 0.8, "course_id": course_name, "document_id": "doc2", "text": "found text 2", "metadata": {"source": "file2.pdf"}},
    ]
